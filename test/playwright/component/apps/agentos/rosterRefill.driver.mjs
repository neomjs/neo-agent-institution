 ...
 * writer shapes the cockpit uses (`clear()` then `add()`, or `clear()` then `load()`), which no
 * remote of the App worker exposes. Every distinct URL executes once (module cache), so a spec
 * varies the `t` parameter per call.
 *
 * `?store=<id>&op=clearAdd&rows=<JSON>&t=<n>` · `?store=<id>&op=clearLoad&t=<n>`
 */
const
