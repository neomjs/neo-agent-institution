...
test.describe('FM cockpit — visual baselines (the design-gate scope floor)', () => {
    ...
    const openGoldenPathPane = async page => {
        const tab = page.locator('.neo-dashboard-dock-tabs .neo-tab-header-button', {hasText: /route graph/i});
    ...
    /**
     * @summary Lands one fixture wire envelope in the cockpit provider from INSIDE the App worker (the
     * Golden Path read's own write, through the test-only driver) and waits for the currency line the
     * pane derives from it — composition and value, never the viewer-clock format; the picture follows
     * one canvas-worker frame later, so a short settle follows.
     * @param {Object} page
     ...
     */
    const feedGoldenPath = async (page, state, currency) => {
        ...
        await expect(page.locator('.fm-goldenpath-graph-pane .fm-goldenpath-currency')).toHaveText(currency);
        await page.waitForTimeout(600)
    };

    test('the Route graph pane — cold it is unavailable on a clean surface; a current route draws the ranked spine with its shared citations; both skins', async ({page}) => {
        await bootSettledCockpit(page);
        await openGoldenPathPane(page);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-unavailable.png');

        await feedGoldenPath(page, 'current', /^Current · captured .+ · 4 items$/);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-current.png');

        // the hover names an item through the canvas mouse path (DOM event → App worker → renderer):
        // the pointer over the rank-2 node, whose position is the layout's — the second of four on
        // the spine between the 40 px paddings, the spine at 38 % of the surface
        const rect = await page.locator('.fm-goldenpath-graph-pane canvas').boundingBox();

        await page.mouse.move(rect.x + 40 + (rect.width - 80) / 3, rect.y + rect.height * 0.38);
        await page.waitForTimeout(400);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-current-hover.png');
        await page.mouse.move(0, 0);
        await page.waitForTimeout(400);

        await switchToLightSkin(page);
        await page.waitForTimeout(600);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-current-light.png')
    });

    test('the Route graph pane — withheld dims the last known good route and names the reason; degraded and unavailable leave the surface clean; both skins', async ({page}) => {
        await bootSettledCockpit(page);
        await openGoldenPathPane(page);

        await feedGoldenPath(page, 'withheld', /^Withheld · freshness-sla-breached · last known good route · captured .+ · 4 items$/);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-withheld.png');

        await feedGoldenPath(page, 'degraded', /^Degraded · route-sidecar-missing$/);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-degraded.png');

        await switchToLightSkin(page);
        await page.waitForTimeout(600);

        await feedGoldenPath(page, 'withheld', /^Withheld · freshness-sla-breached · last known good route · captured .+ · 4 items$/);
        await expect(page.locator('.fm-goldenpath-graph-pane')).toHaveScreenshot('goldenpath-pane-withheld-light.png');
